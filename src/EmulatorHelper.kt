 ...
 */
fun takeAndPullScreenshot(id: String, folderName: String = "docs/results"): String{
...
fun getEmulators(){
    ...
    val commands = listOf("emulator", "-list-avds")
...
fun isHome(): Boolean{
    ...
    return  (actual.contains("launcher") || actual.contains("Launcher"))
}
/**
    ...
    with(answer.first().split("\n")){
        (find { it -> it.contains("launchable-activity") }?: "").let {
            return if(it != ""){
                ...
                throw Exception("There is no launchable activity")
            }
        }
    }
}
...
fun connectDeviceWirelessIP(IPAddress: String, port: String){
    simpleTryCatch {
        var commands = listOf("adb", "tcpip", port)
        var answer = executeProcess2(commands, "OPENING PORT 5556", null, null)
        println("tcpConnection: $answer")

        commands = listOf("adb", "connect","$IPAddress:$port")
        answer = executeProcess2(commands, "CONNECTING PORT $IPAddress",null,null)
        println("IPAddres and port: $answer")
    }
}

fun stopEmulator(emulatorId: String){
    simpleTryCatch {
        var commands = listOf("adb", "-s", "$emulatorId", "emu", "kill")
        var answer = executeProcess2(commands, "OPENING PORT 5556", null, null)
        println(answer)
    }
}
...
fun getDevices(){
    simpleTryCatch {
        var commands = listOf("adb", "devices")
        var answer = executeProcess2(commands, "getting devices", null, null)
        println(answer)
    }
}